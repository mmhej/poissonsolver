...
!------------------------------------------------------------------------------!
	...
	INTEGER,DIMENSION(3),PARAMETER :: domain_ncell  = (/ 64,128,128 /)
	INTEGER,DIMENSION(3),PARAMETER :: domain_bounds = (/ 0,0,0 /)

	REAL(MK),DIMENSION(3),PARAMETER :: domain_xmin = (/ -0.5_MK,-1.0_MK,-1.0_MK /)
	REAL(MK),DIMENSION(3),PARAMETER :: domain_xmax = (/  0.5_MK, 1.0_MK, 1.0_MK /)
...
!------------------------------------------------------------------------------!
	...
	IF (rank .EQ. 0) THEN
		WRITE(*,*)""
		WRITE(*,*)"TESTING UNBOUNDED DOMAIN IN 3D... "
		WRITE(*,*)""
	...
	DO k = 0, ncell(3)-1
		...
		DO j = 0, ncell(2)-1
			...
			DO i = 0, ncell(1)-1
				...
				rho   = SQRT(z*z + y*y)
				phi   = SQRT((rho - r0)**2 + x**2)
				theta = ATAN2(z,y)

				IF ( phi .LT. r0) THEN

					Bmag = -EXP(- c*r0**2/(2.0_MK*r0*rho - rho**2 - x**2)) * &
					       ( 4.0_MK*c**2*r0**4*x**2*rho**2 &
					       - 16.0_MK*r0**4*rho**4 &
					       + 32.0_MK*r0**3*rho**5 &
					       - 24.0_MK*r0**2*rho**6 &
					       + 8.0_MK*r0*rho**7 & 
					       ...
					       + 2.0_MK*c*r0**2*rho**6 &
					       + 32.0_MK*r0**3*rho**3*x**2 &
					       - 48.0_MK*r0**2*rho**4*x**2 &
					       - 24.0_MK*r0**2*rho**2*x**4 & 
					       + 24.0_MK*r0*rho**5*x**2 &
					       + 24.0_MK*r0*rho**3*x**4 & 
					       + 8.0_MK*r0*rho*x**6 & 
					       + 2.0_MK*c*r0**3*rho*x**4 &
					       + 2.0_MK*c*r0**2*rho**2*x**4 &
					       - 4.0_MK*c*r0**3*rho**3*x**2 & 
					       + 4.0_MK*c*r0**2*rho**4*x**2 & 
					       - rho**8 - x**8) & 
					       * (2.0_MK*r0*rho - rho**2 - x**2)**(-4)*rho**(-2)
	...
	DO k = 0, ncell(3)-1
		...
		DO j = 0, ncell(2)-1
			...
			DO i = 0, ncell(1)-1
				...
				IF (phi .LT. r0) THEN
! velocity check
					Amag = 2.0_MK*c*r0**2*x &
					        & * EXP(-c*r0**2/(2.0_MK*r0*rho - rho**2 - x**2)) &
					        & * (2.0_MK*r0*rho - rho**2 - x**2)**(-2)

					solX = EXP(-c*r0**2/(2.0_MK*r0*rho - rho**2 - x**2)) * &
					       & ( 4.0_MK*r0**2 *rho**2 &
					       & - 4.0_MK*r0*rho**3 & 
					       & - 4.0_MK*r0*rho*x**2 &
					       & + rho**4 + 2.0_MK*rho**2*x**2 &
					       & + x**4 + 2.0_MK*c*r0**3*rho &
					       & - 2.0_MK*c*r0**2*rho**2) &
					       & *(2.0_MK*r0*rho - rho**2 - x**2)**(-2)*rho**(-1)
					solY = COS(theta)*Amag
					solZ = SIN(theta)*Amag
