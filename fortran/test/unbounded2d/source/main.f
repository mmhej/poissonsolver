...
!------------------------------------------------------------------------------!
	...
	REAL(MK),PARAMETER :: m  = 4.0_MK

	INTEGER,DIMENSION(2),PARAMETER :: domain_ncell = (/ 128, 128 /)
	INTEGER,DIMENSION(2),PARAMETER :: domain_bounds = (/ 0, 0 /)
...
!------------------------------------------------------------------------------!
	...
	IF (rank .EQ. 0) THEN
		WRITE(*,*)""
		WRITE(*,*)"TESTING UNBOUNDED DOMAIN IN 2D... "
		WRITE(*,*)""
...
!------------------------------------------------------------------------------!
	...
	CALL poisson_solver_set_return_grad( 1, .TRUE. ) ! specify lhs operator
...
!------------------------------------------------------------------------------!
	DO j = 0, ncell(2)-1
		...
		DO i = 0, ncell(1)-1
			...
			IF ( r < r0 ) THEN
				Bx(ij) = 4.0_MK * c * r0**2 &
				       * EXP(- c * r0**2/(r0**2 - x**2 - y**2)) &
				       * ( r0**4 - x**4 - y**4 - 2.0*x**2*y**2 &
				         - c*x**2*r0**2 - c*y**2*r0**2 ) &
				       * (r0**2 - x**2 - y**2)**(-4)
...
!------------------------------------------------------------------------------!
	...
	DO j = 0,ncell(2)-1
		...
		DO i = 0,ncell(1)-1
			...
			IF (r .LT. 0.25_MK*dx(1)) THEN
				solX = 0.0_MK
				solY = 0.0_MK
			ELSE IF( r .LT. r0 ) THEN
				solX = - 2.0_MK * c * r0**2 * x &
				     * EXP( -c * r0**2/(r0**2 - x**2 - y**2)) &
				     * ( r0**2 - x**2 - y**2 )**(-2)

				solY = - 2.0_MK * c * r0**2 * y &
				     * EXP( -c * r0**2/( r0**2 - x**2 - y**2 )) &
				     * ( r0**2 - x**2 - y**2)**(-2)

!				solX = - x * (1.0 - ( 1.0 - r**2 )**(m+1) )/(2.0*(m+1.0)*r**2)
!				solY = - y * (1.0 - ( 1.0 - r**2 )**(m+1) )/(2.0*(m+1.0)*r**2)
			ELSE
				solX = 0.0_MK
				solY = 0.0_MK

!				solX = - x/(2.0*(m+1.0)*r**2)
!				solY = - y/(2.0*(m+1.0)*r**2)
			END IF
